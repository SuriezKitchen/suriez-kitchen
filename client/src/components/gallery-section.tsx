      ...
      <div className="container mx-auto px-4">
        <div className="text-center mb-16 scroll-reveal">
          ...
          <p className="text-xl text-muted-foreground max-w-3xl mx-auto" data-testid="gallery-description">
            Each dish tells a story of passion, precision, and creativity. Explore the artistry behind every plate.
          </p>
        </div>
        
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {dishes?.map((dish, index) => (
            <div key={dish.id} className="scroll-reveal group" style={{ animationDelay: `${index * 0.1}s` }}>
                ...
                <div className="relative overflow-hidden">
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-black/50 to-transparent opacity-0 group-hover:opacity-100 transition-opacity">
                    <div className="absolute bottom-4 left-4 text-white">
                      <i className="fas fa-heart text-lg" data-testid={`dish-heart-${dish.id}`}></i>
                    </div>
                  </div>
                </div>
                <CardContent className="p-6">
                  <h3 className="font-serif text-xl font-semibold mb-2" data-testid={`dish-title-${dish.id}`}>
                    {dish.title}
                  </h3>
                  <p className="text-muted-foreground" data-testid={`dish-description-${dish.id}`}>
                    {dish.description}
                  </p>
                </CardContent>
              </Card>
